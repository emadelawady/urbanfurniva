<x-layout>
    ...
    <x-slot:title>
        {!! config('app.name') !!}- {{ __('messages.officefurniture') }}
    </x-slot>
    ...
    <section class="mt-24">
        ...
        <div class="mx-auto max-w-screen-xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
          <header>
            ...
            <x-heading :link="'javascript:avoid(0)'" class="text-xl font-bold text-gray-900 sm:text-3xl">{{  __('messages.officefurniture') }}</x-heading>
          ...
          <ul class="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            @foreach ($officefurnitures as $office)


            {{-- @dd($room) --}}

            <li class="border rounded-lg mx-4 p-2">
              <a href="{{ route('furniture.officefurniture.show' , ['officefurniture' => $office->id]) }}" class="group block overflow-hidden p-4">

                @foreach ($office->product_image as $of)

                    @if ($loop->first)

                        <img
                        src="{{ asset("storage/$of") }}"
                        alt=""
                ...
                <div class="relative bg-white pt-3 mt-4">
                  ...
                  <p class="mt-2">
                    ...
                    {{-- <span class="tracking-wider text-gray-900 text-xs"> {{ $office->model }} </span> --}}
                  </p>
                </div>
              </a>
            </li>
