<x-layout>
    ...
    <x-slot:title>
        {!! config('app.name') !!} - {{ $entrywayfurniture->title }}
    </x-slot>
...
<div class="min-w-screen min-h-screen flex flex-col items-center p-5 lg:p-10 overflow-hidden relative">
    ...
    <div class="w-full max-w-6xl flex my-12">
        <x-filament::breadcrumbs class="bg-gray-50 p-2 rounded-md" :breadcrumbs="[
            ...
            route('furniture') => __('nav.furniture'),
            route('furniture.entrywayfurniture') => __('messages.entrywayfurniture'),
            'javascript:avoid(0)' => $entrywayfurniture->title,
            // '' => '',
        ]" />
    </div>
            ...
            <div class="w-full md:w-1/2 px-10">
                    <div class="mb-10 *:leading-10 text-center *:text-app-orange">
                        <h1 class="font-bold uppercase mb-10 text-3xl leading-none align-baseline !text-app-orange-2">
                            {{ $entrywayfurniture->title }}
                        </h1>

                        {!! $entrywayfurniture->content !!}
                    ...
                    <div class="w-full my-4">
                        ...
                        <div class="flex my-2">
                            ...
                            <p class="mx-4 text-gray-500">
                                {{ $entrywayfurniture->upholstery_material }}
                            </p>
                        </div>
                        <div class="flex my-2">
                            <span class="text-app-orange-2">
            ...
            <div class="w-full md:w-1/2 px-10 mb-10 md:mb-0">
                <div class="relative">
                    ...
                    <x-front.furniture.livingroom.sofa.show-page-slider
                        :title="$entrywayfurniture->title"
                        :model="collect($entrywayfurniture->product_image)->take(4)"
                        :swiperClass="'furniture-entrywayfurniture-product-prev'"
                        :swiperThumb="'furniture-entrywayfurniture-product-thumb'"
...
<div class="max-w-screen-xl mx-auto p-5 sm:p-10 md:p-16">
    ...
    <div class="border-b mb-5 flex justify-between text-sm">
        ...
        <a href="{{ route('furniture.entrywayfurniture') }}" class="hover:text-indigo-600">مشاهدة الكل</a>
    </div>


    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">

        @foreach ($entrywayfurnituresAll as $e_r_a)

        <!-- CARD 1 -->
        <div class="rounded overflow-hidden shadow-lg flex flex-col">
            {{-- <a href="#"></a> --}}
            <div class="relative">
                <a href="{{ route('furniture.officefurniture.show', ['officefurniture' => $e_r_a->id]) }}">


                    @foreach ($e_r_a->product_image as $ero)

                        @if ($loop->first)

                            <img
                            src="{{ asset("storage/$ero") }}"
                            alt="{{ $e_r_a->title }}"
                            class="w-full h-64"
                            />
                        @endif
