<x-layout>
    ...
    <x-slot:title>
        {!! config('app.name') !!}- {{ __('messages.tables') }}
    </x-slot>
        ...
        <div class="mx-auto max-w-screen-xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
          ...
          <ul class="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <li class="border rounded-lg mx-4 p-2">
              <a href="{{ route('furniture.livingroom.table.show', ['table' => $table->id]) }}" class="group block overflow-hidden p-4">

                @foreach ($table->product_image as $ta)

                    @if ($loop->first)

                        <img
                        src="{{ asset("storage/$ta") }}"
                        alt=""
                ...
                <div class="relative bg-white pt-3 mt-4">
                  ...
                  <p class="mt-2">
                    ...
                    {{-- <span class="tracking-wider text-gray-900 text-xs"> {{ $table->model }} </span> --}}
                  </p>
                </div>
              </a>
            </li>
