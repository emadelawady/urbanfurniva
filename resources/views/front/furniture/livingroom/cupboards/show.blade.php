<x-layout>
    ...
    <x-slot:title>
        {!! config('app.name') !!} - {{ $cupboard->title }}
    </x-slot>
...
<div class="min-w-screen min-h-screen flex flex-col items-center p-5 lg:p-10 overflow-hidden relative">
    ...
    <div class="w-full max-w-6xl flex my-12">
        <x-filament::breadcrumbs class="bg-gray-50 p-2 rounded-md" :breadcrumbs="[
            ...
            route('furniture.livingroom.cupboard') => __('messages.cupboards'),
            'javascript:avoid(0)' => $cupboard->title,
            // '' => '',
        ]" />
    </div>
            ...
            <div class="w-full md:w-1/2 px-10 mb-10 md:mb-0">
                <div class="relative">
                    ...
                    <x-front.furniture.livingroom.sofa.show-page-slider
                        ...
                        :model="collect($cupboard->product_image)->take(4)"
                        :swiperClass="'furniture-livingroom-cupboard-product-prev'"
                        :swiperThumb="'furniture-livingroom-cupboard-product-thumb'"
...
<div class="max-w-screen-xl mx-auto p-5 sm:p-10 md:p-16">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
        ...
        @foreach ($cupboardsAll as $c_b_a)

        <!-- CARD 1 -->
        <div class="rounded overflow-hidden shadow-lg flex flex-col">
            {{-- <a href="#"></a> --}}
            <div class="relative">
                <a href="{{ route('furniture.livingroom.cupboard.show', ['cupboard' => $c_b_a->id]) }}">


                    @foreach ($c_b_a->product_image as $cba)

                        @if ($loop->first)

                            <img
                            src="{{ asset("storage/$cba") }}"
                            alt="{{ $c_b_a->title }}"
                            class="w-full h-64"
                            />
                        @endif
