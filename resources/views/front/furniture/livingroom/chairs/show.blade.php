...
<div class="min-w-screen min-h-screen flex flex-col items-center p-5 lg:p-10 overflow-hidden relative">
    ...
    <div class="w-full max-w-6xl flex my-12">
        <x-filament::breadcrumbs class="bg-gray-50 p-2 rounded-md" :breadcrumbs="[
            ...
            route('furniture.livingroom.chair') => __('messages.chairs'),
            'javascript:avoid(0)' => $chair->title,
            // '' => '',
        ]" />
    </div>
            ...
            <div class="w-full md:w-1/2 px-10 mb-10 md:mb-0">
                <div class="relative">
                    ...
                    <x-front.furniture.livingroom.sofa.show-page-slider
                        :title="$chair->title"
                        :model="collect($chair->product_image)->take(4)"
                        :swiperClass="'furniture-livingroom-chair-product-prev'"
                        :swiperThumb="'furniture-livingroom-chair-product-thumb'"
...
<div class="max-w-screen-xl mx-auto p-5 sm:p-10 md:p-16">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
        ...
        @foreach ($chairsAll as $ch_air)

        <!-- CARD 1 -->
        <div class="rounded overflow-hidden shadow-lg flex flex-col">
            {{-- <a href="#"></a> --}}
            <div class="relative">
                <a href="{{ route('furniture.livingroom.chair.show', ['chair' => $ch_air->id]) }}">


                    @foreach ($ch_air->product_image as $c_h_a)

                        @if ($loop->first)

                            <img
                            src="{{ asset("storage/$c_h_a") }}"
                            alt="{{ $ch_air->title }}"
                            class="w-full h-64"
                            />
                        @endif
