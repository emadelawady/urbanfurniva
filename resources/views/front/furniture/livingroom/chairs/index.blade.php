<x-layout>
    ...
    <x-slot:title>
        {!! config('app.name') !!}- {{ __('messages.chairs') }}
    </x-slot>
    ...
    <section class="mt-24">
        ...
        <div class="mx-auto max-w-screen-xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
          <header>
            ...
            <x-heading :link="'javascript:avoid(0)'" class="text-xl font-bold text-gray-900 sm:text-3xl">{{  __('messages.chairs') }}</x-heading>
          ...
          <ul class="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <li class="border rounded-lg mx-4 p-2">
              <a href="{{ route('furniture.livingroom.chair.show', ['chair' => $chair->id]) }}" class="group block overflow-hidden p-4">

                @foreach ($chair->product_image as $ch)

                    @if ($loop->first)

                        <img
                        src="{{ asset("storage/$ch") }}"
                        alt=""
                ...
                <div class="relative bg-white pt-3 mt-4">
                  ...
                  {{-- <p class="mt-2">
                    ...
                    <span class="tracking-wider text-gray-900 text-xs"> {{ $chair->model }} </span>
                  </p> --}}
