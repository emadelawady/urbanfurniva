...
"
class="fixed top-0 left-0 w-full h-auto py-2 duration-300 ease-out bg-green-500 text-white shadow-sm sm:py-0 sm:h-10 z-50" x-cloak>
<div class="flex items-center justify-between w-full h-full px-3 mx-auto max-w-7xl ">
    <a href="javascript:avoid(0)" class="flex flex-col w-full h-full text-md leading-6 text-white duration-150 ease-out sm:flex-row sm:items-center opacity-80 hover:opacity-100">
    ...
    <button @click="bannerVisible=false; setTimeout(()=>{ bannerVisible = true }, 1000);" class="flex items-center flex-shrink-0 translate-x-1 ease-out duration-150 justify-center w-6 h-6 p-1.5 text-white rounded-full hover:bg-neutral-100">
