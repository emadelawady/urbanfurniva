<section class="flex flex-col items-center justify-center">
    ...
    <div class="w-full">
        ...
        <h2 class="text-4xl font-extrabold text-gray-800 mb-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 max-xl:gap-4 gap-6 py-8">
                ...
                @foreach ($burnergas as $b_g)

                    <a  href="{{ route('burner.gas.show', ['burnerGas' => $b_g->id]) }}" class="bg-white rounded-2xl p-5 cursor-pointer hover:-translate-y-2 transition-all relative hover:ring-1 hover:ring-red-200">
                        ...
                        <div class="w-5/6 h-[150px] overflow-hidden mx-auto aspect-w-8 aspect-h-4 md:mb-2 mb-4">
                            <img src="{{ asset("storage/$b_g->product_image") }}" alt="Product 1"
                            class="h-full w-full object-contain" />
                        </div>

                        <div>
                            <h3 class="text-lg font-extrabold text-gray-800">{{ $b_g->title }}</h3>
    ...
    <x-pri-button :link="route('burner.gas')">{{ __('messages.learn-more') }}</x-pri-button>
